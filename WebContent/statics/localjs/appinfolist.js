$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/Appsys/dev/categorylevellistajax",//请求的url
...
$("#queryCategoryLevel2").change(function(){
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/Appsys/dev/categorylevellistajax2",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/Appsys/dev/versionadd?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/Appsys/dev/versionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/Appsys/dev/appinfupdate?id="+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	$.ajax({
		type:"get",
		url:"/Appsys/dev/sxajax?appId="+ appId,
		dataType:"json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="/Appsys/dev/appview?id="+ obj.attr("appinfoid");
});
